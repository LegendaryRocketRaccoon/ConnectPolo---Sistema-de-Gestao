...
async function inicializar() {
    try {
        ...
        configurarAutoPreenchimento(); // ✨ NOVO
...
async function carregarTurmas() {
    ...
    try {
        ...
        // ORDENAR: Auxiliar de Estatística SEMPRE em primeiro
        turmasDisponiveis.sort((a, b) => {
            if (a.nome.includes("Auxiliar de Estatística")) return -1;
            if (b.nome.includes("Auxiliar de Estatística")) return 1;
            return a.nome.localeCompare(b.nome);
        });
        
        console.log("📚 [5/5] Turmas ordenadas (Auxiliar de Estatística primeiro)");
...
function popularSelectTurmas(erro = false) {
    ...
    // ✅ Auxiliar de Estatística já está em primeiro (ordenado acima)
...
// ===== AUTO-PREENCHIMENTO DO NOME DE EXIBIÇÃO =====
function configurarAutoPreenchimento() {
    ...
    btnAuto.addEventListener('click', () => {
        ...
        // ✅ CORRIGIDO: Pega PRIMEIRO + ÚLTIMO nome
        ...
        if (palavras.length === 1) {
            ...
            // Tem 3+ palavras: pega primeiro + último
...
async function handleSubmit(e) {
    ...
    try {
        ...
        // Usar o novo serviço do Firestore
...
⚠️ IMPORTANTE: Anote essa senha e envie ao aluno.
O aluno deverá usar essas credenciais para fazer login.
                `.trim();
                
                // Mostrar em alert para garantir que o usuário veja
                setTimeout(() => {
                    alert(senhaMsg);
                    
                    // Copiar para clipboard
                    const credenciais = `Email: ${result.data.email}\nSenha: ${result.data.senhaTemporaria}`;
                    navigator.clipboard.writeText(credenciais)
                        .then(() => {
                            showMessage('✅ Credenciais copiadas para área de transferência.', 'success');
                        })
                        .catch(() => {
